...
- **source-control/** - git status / stage / commit panel and diff workflow. Status refreshes on context change (2 s TTL), on window focus, after any git action, and on `fs:changed` from the explorer/editor watchers (`shouldRefreshForPaths` drops batches outside the repo or inside `.git`; `fsRefreshScheduler` debounces 500 ms, keeps reads 1.5 s apart, defers while the window is hidden, and never forgets a change until a read that started after it completes). No polling.
